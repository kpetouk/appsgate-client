        ...
        validEditService: function(e) {
            ...
            if (e.type === "keyup" && e.keyCode === 13 || e.type === "click") {
                ...
                if (this.checkService()) {
                    ...
                    if (this.model.get("type") !== "21" && this.model.get("type") !== 21) {
                        destPlaceId = $("#edit-service-modal select option:selected").val();
                    }
                    ...
                    this.$el.find("#edit-service-modal").on("hidden.bs.modal", function() {
                        ...
                        // move the service if this is not the core clock
                        if (self.model.get("type") !== "21" && self.model.get("type") !== 21) {
                            places.moveService(self.model.get("placeId"), destPlaceId, self.model.get("id"), true);
                        } else { // update the time and the flow rate set by the user
                            // update the moment attribute
                            self.model.get("moment").set("hour", parseInt($("#edit-service-modal select#hour").val()));
                            self.model.get("moment").set("minute", parseInt($("#edit-service-modal select#minute").val()));
                            // retrieve the value of the flow rate set by the user
                            var timeFlowRate = $("#edit-service-modal input#time-flow-rate").val();
