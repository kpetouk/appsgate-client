define([
...
], function(App, Grammar, actionNodeTemplate, ifNodeTemplate, whenNodeTemplate, deviceNodeTemplate, eventNodeTemplate) {
    ...
    ProgramMediator = Backbone.Model.extend({
                ...
                for(t in nodes) {
                    switch(nodes[t]) {
                        ...
                        case '"event"':
                            ...
                            break;
                        case "ID":
                            console.error("empty program");
                            break;

                        default:
                            console.warn(nodes[t] + "not implemented right now");
                            break;
                    }
                }
            } else {
        ...
        buildInputFrom: function(jsonNode) {
            param = {node:jsonNode, engine:this};
            switch (jsonNode.type) {
                case "action":
                    return this.tplActionNode(param);
                case "if":
                    return this.tplIfNode(param);
                case "when":
                    return this.tplWhenNode(param);
                case "device":
                    return this.tplDeviceNode(param);
                case "event":
                    return this.tplEventNode(param);
                case "empty":
                    return "<button class='btn btn-prog' id='" + jsonNode.iid + "'></button>";
                default:
                    return "<button class='btn btn-prog' id='" + jsonNode.iid + "'><span>" + jsonNode.type +"</span></button>";
            }
            return "";
        },
        ...
        buildInputFromJSON: function() {
            ...
            $(".programInput").html(this.buildInputFrom(this.programJSON));
        },
